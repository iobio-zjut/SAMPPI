...
def all_feature(ls, names, mutations,ddG,variable_name, server_dir):
    all = []
    index = 0

    base_dir = f"{server_dir}/data/M1101_PSSM"
    pssm_dir = os.path.join(base_dir, variable_name)

    base_ESM2_dir = f"{server_dir}/data/M1101_esm2_emb"
    pkl_dir = os.path.join(base_ESM2_dir, variable_name)

    base_antiberty_dir = f"{server_dir}/data/M1101_antiberty"
    antiberty_dir = os.path.join(base_antiberty_dir, variable_name)

    base_prott5_dir = f"{server_dir}/data/M1101_prott5"
    prott5_pkl_dir = os.path.join(base_prott5_dir, variable_name)

    base_sec_dir = f"{server_dir}/data/M1101_secondary_structure"
    ...
    for s in ls:
        if s == s:  # 检查序列是否为 NaN
            ...
            f6=feature(s).seq2Blosum62(f'{server_dir}/data/blosum62.txt') #[L,23]
            ...
            f8=feature(s).meiler(f'{server_dir}/data/Meiler.csv')
...
if __name__ == "__main__":
    ...
    csv = pd.read_csv(f'{base_dir}/data/M1101.csv',usecols=['PDB','mutation_clean','antibody_light_seq','antibody_heavy_seq','antigen_a_seq','antigen_b_seq','antibody_light_seq_mut','antibody_heavy_seq_mut','antigen_a_seq_mut','antigen_b_seq_mut', 'ddG'])
    ...
    antibodies_l = all_feature(abls,names,mutations,labels,"ab_l", base_dir)
    antibodies_h = all_feature(abhs,names,mutations,labels,"ab_h", base_dir)
    antigens_a = all_feature(agas,names,mutations,labels,"ag_a", base_dir)
    antigens_b = all_feature(agbs,names,mutations,labels,"ag_b", base_dir)
    antibodies_l_mut = all_feature(abls_m,names,mutations,labels,"ab_l_m", base_dir)
    antibodies_h_mut = all_feature(abhs_m,names,mutations,labels,"ab_h_m", base_dir)
    antigens_a_mut = all_feature(agas_m,names,mutations,labels,"ag_a_m", base_dir)
    antigens_b_mut = all_feature(agbs_m,names,mutations,labels,"ag_b_m", base_dir)
    ...
    for train_index, val_index in kf.split(interactions):
        ...
        decoder_ab = Decoder(ab_dim, hid_dim, n_layers, n_heads, pf_dim, DecoderLayer, SelfAttention, PositionwiseFeedforward, dropout, device)
        decoder_ag = Decoder(ag_dim, hid_dim, n_layers, n_heads, pf_dim, DecoderLayer, SelfAttention, PositionwiseFeedforward, dropout, device)

        model = Predictor(encoder_ab, encoder_ag, decoder_ab, decoder_ag, device)
        model.to(device)


        trainer = Trainer(model, lr, weight_decay, batch, ab_dim,ag_dim)
        tester = Tester(model, device, ab_dim,ag_dim)

        print('*************************** start training on Fold %s ***************************'%i)

        # 定义目标路径
        output_dir = f'{base_dir}/output/M1101'
        ...
        file_best_pcc_PCCS = f'{base_dir}/output/M1101/RECORD_{}.txt'.format(i)
        file_best_pcc_model = f'{base_dir}/output/M1101/model_{}'.format(i)
        ...
        for epoch in range(1, iteration + 1):
            ...
            print('Epoch:',epoch)
            loss_train_fold, y_train_true, y_train_predict = trainer.train(dataset_train, device)  # numpy arrays record for an epoch loss.
            pccs_val, mae_val, mse_val, rmse_val, r2_val, loss_val_fold, y_val_true, y_val_predict = tester.test(dataset_val)  # pccs_dev && loss_val_fold are for an epoch
            ...
            if pccs_val.tolist() > best_pearson:
                print(f'Validation PCCS decreased ({best_pearson:.4f} --> {pccs_val.tolist():.4f}).  Saving model ...')
