...
class Predictor(nn.Module):
    def __init__(self, encoder_ab, encoder_ag, decoder_ab, decoder_ag, device):
        ...
        self.f9_linear=nn.Linear(1,1)###zheli
        self.f10_linear=nn.Linear(7,7)
    ...
    def process_tensor(self, x):
        ...
        # f1, f2, f3 = x[:, :, :20], x[:, :, 20:40], x[:, :, 40:1320]  # 拆分特征
        f1, f2, f3, f6,f8,f9,f10 = x[:,:, :20], x[:, :, 20:40], x[:, :, 40:1320], x[:, :, 1320:2344],x[:,:,2344:2367],x[:,:,2367:2368],x[:,:,2368:2375]
        f1_proj = self.f1_linear(f1)  # 线性变换
        f2_proj = self.f2_linear(f2)
        f3_proj = self.f3_linear(f3)
        f6_proj = self.f6_linear(f6)
        f8_proj=self.f8_linear(f8)
        f9_proj=self.f9_linear(f9)
        f10_proj = self.f10_linear(f10)

        out=torch.cat([f1_proj, f2_proj, f3_proj,f6_proj,f8_proj,f9_proj,f10_proj], dim=-1)  # 重新拼接
...
# zero pack
def pack(ag_s, ab_s, ag_m_s, ab_m_s, labels, device,protein_dim):
    ...
    ag_s_new = torch.zeros((N, ag_s_len, protein_dim), device=device)
    i = 0
    for ag in ag_s:
        #ag = ag.astype(float)
        ag = torch.tensor(ag)
        ...
        ag_s_new[i, :a_len, :] = ag
        i += 1
    ab_s_new = torch.zeros((N, ab_s_len, protein_dim), device=device)
    i = 0
    for ab in ab_s:
        #ab = ab.astype(float)
        ab = torch.tensor(ab)
    ...
    ag_m_s_new = torch.zeros((N, ag_m_s_len, protein_dim), device=device)
    i = 0
    for agm in ag_m_s:
        #ag = ag.astype(float)
        ...
        i += 1
    ab_m_s_new = torch.zeros((N, ab_m_s_len, protein_dim), device=device)
    i = 0
    for abm in ab_m_s:
        #ab = ab.astype(float)
    ...
    def train(self, dataset, device):
        ...
        for data in dataset:
            ...
            if i % self.batch == 0 or i == N:
                iteration += 1
                data_pack = pack(ag_s, ab_s, ag_m_s, ab_m_s, labels, device,self.protein_dim)
                loss, correct, predicted,predicted_reversed = self.model(data_pack)  # predictor.train()
...
class Tester(object):
    ...
    def test(self, dataset):
        ...
        Y_reversed = torch.zeros((1, 0), device=self.device)  # 使用传入的 device
        with torch.no_grad():
            for data in dataset:
                ag_s, ab_s, ag_m_s, ab_m_s, labels = [], [], [], [], []
                ag, ab, agm, abm, label = data
                ...
                ab_m_s.append(abm)
                labels.append(label)
                data = pack(ag_s, ab_s, ag_m_s, ab_m_s, labels, self.device,self.protein_dim)  # 使用传入的 device
                correct, predicted,predicted_reversed = self.model(data, train=False)
                correct = correct.view(1,-1)
                predicted = predicted.view(1,-1)
                predicted_reversed = predicted_reversed.view(1,-1)
                T = torch.cat([T,correct], dim=-1)
                Y = torch.cat([Y,predicted], dim=-1)
                Y_reversed = torch.cat([Y_reversed,predicted_reversed], dim=-1)
        T = T.squeeze()
        Y = Y.squeeze()
        T_reversed = -T
        Y_reversed = Y_reversed.squeeze()
        Loss = nn.MSELoss()
        ...
        T = T.detach().cpu().numpy()  # 如果 T 是 PyTorch 张量，先转 numpy 数组
        Y = Y.detach().cpu().numpy()
        mae = mean_absolute_error(T, Y)
        mse = mean_squared_error(T, Y)
        rmse = sqrt(mean_squared_error(T, Y))
        r2 = r2_score(T, Y)
        return pccs,pccs_reversed, mae, mse, rmse, r2, loss, T, Y
